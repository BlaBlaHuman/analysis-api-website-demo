...
<span class="kd">interface </span><a href="index.html">KaSmartCastSource</a> : <a href="../../org.jetbrains.kotlin.analysis.api.lifetime/-ka-lifetime-owner/index.html">KaLifetimeOwner</a>
</span></code></pre></div>Either a symbol for which a smart cast is generated, or one of its receivers.
...
<div class="highlight"><pre><code class="md-code__content"><span markdown><span class="kd">abstract </span><span class="kd">val </span><a href="index.html#dispatchreceiver">dispatchReceiver</a><span class="o">: </span><a href="index.html">KaSmartCastSource</a><span class="o">?</span>
</span></code></pre></div>
</div>
...
The [`symbol`](index.md#symbol) only participates in the smart cast if its dispatch receiver is the same as this one. E.g.:
...
In the example above, the `holder` parameter is a required dispatch receiver for `value`. If any other receiver is passed, such as `Holder().value`, the smart cast is not generated.
...
<div class="highlight"><pre><code class="md-code__content"><span markdown><span class="kd">abstract </span><span class="kd">val </span><a href="index.html#extensionreceiver">extensionReceiver</a><span class="o">: </span><a href="index.html">KaSmartCastSource</a><span class="o">?</span>
</span></code></pre></div>
</div>
...
The original type of the [`symbol`](index.md#symbol).
...
<div class="highlight"><pre><code class="md-code__content"><span markdown><span class="kd">abstract </span><span class="kd">val </span><a href="index.html#symbol">symbol</a><span class="o">: </span><a href="../../org.jetbrains.kotlin.analysis.api.symbols/-ka-declaration-symbol/index.html">KaDeclarationSymbol</a>
</span></code></pre></div>
</div>
...
Smart casts to the same declaration can be allowed or prohibited, depending on how that declaration is accessed. In the following example, the [`symbol`](index.md#symbol) will be `val value: Any`. However, a smart cast will only be produced for `a.value`.
...
For `a.value` from the example above, a [`KaSmartCastSource`](index.md) is generated whose [`symbol`](index.md#symbol) points to `val value: Any` (a property), and whose [`dispatchReceiver`](index.md#dispatchreceiver) is the `a: Holder` value parameter.

For `this`, the [`symbol`](index.md#symbol) may be either a [`org.jetbrains.kotlin.analysis.api.symbols.KaReceiverParameterSymbol`](../../org.jetbrains.kotlin.analysis.api.symbols/-ka-receiver-parameter-symbol/index.md) (for an extension receiver parameter) or a [`org.jetbrains.kotlin.analysis.api.symbols.KaClassSymbol`](../../org.jetbrains.kotlin.analysis.api.symbols/-ka-class-symbol/index.md) (if the cast occurs right inside a class).
